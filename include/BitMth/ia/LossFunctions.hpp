...
namespace BitMth::ia{
    ...
    struct [[nodiscard]] LossFunctions{
      ...
      public:
        ...
        static Matrix mseDerivative(const Matrix& predict, const Matrix& real,T gradScal = static_cast<T>(1), core::Arena* targetArena = nullptr){
            CHECK_ERROR_MATRIX(
                predict.getRows() != real.getRows() || predict.getCols() != real.getCols(),
            ...
            const T invN = (T(2) * gradScal) / static_cast<T>(real.size());
        ...
        static Matrix bceDerivative(const Matrix& predict, const Matrix& real,T gradScal = static_cast<T>(1), core::Arena* targetArena = nullptr) {
            CHECK_ERROR_MATRIX(
                predict.getRows() != real.getRows() || predict.getCols() != real.getCols(),
            ...
            constexpr T one = static_cast<T>(1);
            const T N = gradScal / static_cast<T>(real.size());
