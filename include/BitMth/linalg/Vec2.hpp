...
namespace BitMth::linalg{
    ...
    class [[nodiscard]] Vec2{
        ...
        Vec2<T> operator-(T number) const { return  Vec2<T>(x - number, y - number); }
        friend Vec2<T> operator-(T number, const Vec2<T>& vector) { return  Vec2<T>(number - vector.x, number - vector.y); }
        Vec2<T>& operator-=(T number) { 
            x -= number;
            y -= number;
            return *this; 
        ...
        Vec2<T> operator/(T number) const{ 
            CHECK_ERROR_VECTOR(
                number < utils::EPSILON<T> || number > -utils::EPSILON<T>,
                "Vector division (/)", "Division by zero", "2d");
            T inv = T(1) / number;
